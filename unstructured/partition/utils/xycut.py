...
def projection_by_bboxes(boxes: np.ndarray, axis: int) -> np.ndarray:
    """
    Obtain the projection histogram through a set of bounding boxes (bbox),
    and output it in per-pixel form.

    Args:
        boxes: [N, 4]
        axis: 0 - project along the x-axis horizontally, 1 - project along the y-axis vertically

    Returns:
        1D projection histogram with length equal to the maximum coordinate in the projection direction.
        (We don't need the actual length of the image, as we're only interested in finding the gaps between text boxes)
    """
    assert axis in [0, 1]
...
def recursive_xy_cut(boxes: np.ndarray, indices: np.ndarray, res: List[int]):
    ...
    Args:
        boxes: (N, 4) - N bounding boxes with 4 coordinates each
        indices: These indices represent the index of each bounding box in the original data during recursion.
    ...
    for r0, r1 in zip(arr_y0, arr_y1):
        # [r0, r1] indicates that the regions with bboxes are cut horizontally and then vertically.
        ...
        arr_x0, arr_x1 = pos_x
        if len(arr_x0) == 1:
            # x-direction cannot be cut
            res.extend(x_sorted_indices_chunk)
            continue

        # x-direction can be cut, continue with recursive call
